                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div className="bg-white p-8 rounded-2xl border border-stone-200/30 shadow-sm flex gap-4 items-start">
                            <div className="w-12 h-12 rounded-xl bg-amber-550/10 text-amber-900 flex items-center justify-center flex-shrink-0 text-2xl" style={{ backgroundColor: 'rgba(120,53,4,0.1)' }}>🌲</div>
                        ...
                        {/* Benefit 2 */}
                        <div className="bg-white p-8 rounded-2xl border border-stone-200/30 shadow-sm flex gap-4 items-start">
                            <div className="w-12 h-12 rounded-xl bg-amber-550/10 text-amber-900 flex items-center justify-center flex-shrink-0 text-2xl" style={{ backgroundColor: 'rgba(120,53,4,0.1)' }}>🛠️</div>
                            <div className="space-y-2">
            ...
            <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
                ...
                {topProducts && topProducts.length > 0 ? (
                    ...
                    <div className="bg-white p-12 text-center rounded-2xl border border-stone-200/80 shadow-sm space-y-4 max-w-md mx-auto">
                        ...
                        <h3 className="text-lg font-bold text-stone-850">Katalog Produk Masih Kosong</h3>
            ...
            {reviews && reviews.length > 0 && (
                <section className="py-20 bg-stone-100 border-t border-stone-200/50">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {reviews.map((rev) => (
                                <div key={rev.id} className="bg-white p-8 rounded-2xl border border-stone-200/30 shadow-sm flex flex-col justify-between">
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex gap-1 text-amber-500">
                                            {Array.from({ length: 5 }).map((_, i) => (
                                                <svg 
                                                    key={i} 
                                                    className={`w-4 h-4 fill-current ${i < rev.rating ? 'text-amber-500' : 'text-stone-200'}`} 
                                                    viewBox="0 0 20 20"
                                                >
                                    ...
                                    <div className="mt-6 pt-4 border-t border-stone-100 flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <span className="text-xs text-stone-400">{rev.city}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
